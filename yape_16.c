...
// Extender fórmula a toda la columna correctamente
void fill_formula_column(int col) {
    ...
    for (int i = 1; i < nrows; i++) {  // empezar desde fila 1
        char tmp[FORMULA_MAX];
        int pos = 0;

        tmp[pos++] = '=';  // iniciar fórmula
        tmp[pos] = '\0';

        for (int k = 1; base[k] != '\0' && pos < FORMULA_MAX - 1; ) {
            // detectar letra de columna
            if (base[k] >= 'A' && base[k] <= 'Z') {
                // copiar letra de columna
                tmp[pos++] = base[k++];
                tmp[pos] = '\0';

                // saltar cualquier número existente en base
                while (base[k] >= '0' && base[k] <= '9') k++;

                // agregar número de fila correcto
                char row_str[16];
                int len = snprintf(row_str, sizeof(row_str), "%d", i+1);
            ...
            } else {  // operador u otro carácter
                tmp[pos++] = base[k++];
                tmp[pos] = '\0';
            }
        }

        // copiar resultado a la celda
