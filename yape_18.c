...
int edit_mode = 0; // modo edición de celda
char edit_buffer[CELL_LEN]; // buffer temporal para edición
...
void fill_formula_column(int col) {
    ...
    int base_row = cur_row;  // fila donde está la fórmula original
...
int main() {
    ...
    while (1) {
        ...
        if (!formula_mode && !edit_mode) { // Modo normal
            switch(ch) {
                ...
                case '=':
                    ...
                    break;
                case 'e': // Activar edición de celda
                    edit_mode = 1;
        ...
        else if (edit_mode) { // Modo edición de celda
            if (ch == 27) { // Esc cancela edición
                edit_mode = 0;
            } else if (ch == '\n') { // Enter guarda y mueve abajo
                strncpy(sheet[cur_row][cur_col].data, edit_buffer, CELL_LEN-1);
                sheet[cur_row][cur_col].data[CELL_LEN-1] = '\0';
                edit_mode = 0;
                if (cur_row < nrows-1) cur_row++; // avanzar a siguiente fila
