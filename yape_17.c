...
void fill_formula_column(int col) {
    ...
    // Extraer las referencias y ajustar filas
    for (int i = 0; i < nrows; i++) {
        if (i == base_row) continue;  // no sobrescribir la fórmula base

        char tmp[FORMULA_MAX];
        int pos = 0;
        tmp[pos++] = '=';  // iniciar la fórmula
        tmp[pos] = '\0';
        ...
        for (int k = 1; base[k] != '\0' && pos < FORMULA_MAX - 1; ) {
            if (base[k] >= 'A' && base[k] <= 'Z') {
                ...
                // Capturar el número de fila original
                int row_num = 0;
                while (base[k] >= '0' && base[k] <= '9') {
                    row_num = row_num * 10 + (base[k++] - '0');
                }

                // Ajustar la fila relativa: fila actual - fila base + fila original
                ...
                // Copiar operadores u otros caracteres tal cual
                tmp[pos++] = base[k++];
                tmp[pos] = '\0';
            }
        }

        // Guardar fórmula ajustada en la celda
