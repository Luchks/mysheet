...
void sheet_insert_col(Sheet *s, int pos) {
    ...
    for (int i = 0; i < s->nrows; i++) {
        s->cells[i] = realloc(s->cells[i], (s->ncols + 1) * sizeof(char*));
        // desplazar columnas hacia la derecha
        for (int j = s->ncols; j > pos; j--) {
            s->cells[i][j] = s->cells[i][j - 1];
        }
        s->cells[i][pos] = NULL; // inicializar nueva celda
    }
    s->ncols++;
}
...
void sheet_print_ncurses(Sheet *s, int crow, int ccol) {
    ...
    mvprintw(s->nrows + 1, 0, "hjkl: mover | a: agregar al final | i: insertar aquí | e: editar | q: salir");
    refresh();
}
...
int main() {
    ...
    while ((ch = getch()) != 'q') {
        switch (ch) {
            ...
            case 'i': sheet_insert_col(s, ccol); break; // insertar en posición actual
