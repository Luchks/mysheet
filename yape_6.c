...
void sheet_remove_col(Sheet *s, int pos) {
    if (s->ncols == 0 || pos < 0 || pos >= s->ncols) return;
    for (int i = 0; i < s->nrows; i++) {
        free(s->cells[i][pos]);
        for (int j = pos; j < s->ncols - 1; j++) {
            s->cells[i][j] = s->cells[i][j + 1];
        }
        s->cells[i] = realloc(s->cells[i], (s->ncols - 1) * sizeof(char*));
    }
...
void sheet_remove_row(Sheet *s, int pos) {
    if (s->nrows == 0 || pos < 0 || pos >= s->nrows) return;
    for (int j = 0; j < s->ncols; j++) {
        free(s->cells[pos][j]);
    }
    free(s->cells[pos]);
    for (int i = pos; i < s->nrows - 1; i++) {
        s->cells[i] = s->cells[i + 1];
    }
    s->nrows--;
...
void sheet_print_ncurses(Sheet *s, int crow, int ccol) {
    ...
    mvprintw(s->nrows + 1, 0, "hjkl: mover | a: col final | i: ins col | I: ins fila | d: borrar col | D: borrar fila | e: editar | q: salir");
    refresh();
}
...
int main() {
    ...
    while ((ch = getch()) != 'q') {
        switch (ch) {
            ...
            case 'a': sheet_add_col(s); break;
            case 'i': sheet_insert_col(s, ccol); break;
            case 'I': sheet_insert_row(s, crow); break;
            case 'd': sheet_remove_col(s, ccol); if (ccol >= s->ncols) ccol = s->ncols - 1; break;
            case 'D': sheet_remove_row(s, crow); if (crow >= s->nrows) crow = s->nrows - 1; break;
            case 'e': {
                echo();
                char buffer[128];
